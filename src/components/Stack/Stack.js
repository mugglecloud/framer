...
export const Stack = React.memo(function Stack(_a) {
  var {
      direction = "vertical",
      distribution = "start",
      alignment = "center",
  ...
  const gapChildren = wrapInGapElement(
    fractionChildren,
    gap,
    flexDirection,
    justifyContent
  ...
  return React.createElement(
    FrameWithMotion,
    Object.assign({ background: background }, containerProps, {
...
function isGapEnabled(gap, justifyContent) {
  ...
  if (
    justifyContent &&
    ["space-between", "space-around", "space-evenly", "stretch"].indexOf(
      justifyContent
    ) !== -1
  ) {
    return false;
...
function wrapInGapElement(children, gap, direction, justifyContent) {
  ...
  return React.createElement(
    "div",
    { "data-framer-stack-gap": true, style: gapStyle },
    children
  );
}
