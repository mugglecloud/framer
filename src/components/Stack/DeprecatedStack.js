...
import {
  DeprecatedFrame as DeprecatedCoreFrame,
  DeprecatedFrameWithEvents as DeprecatedFrame,
...
export class DeprecatedStack extends Layer {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    const size = StackLayout.calcUpdatedStackComponentSize(
      nextProps,
      prevState
    );
    ...
    if (nextBackgroundImageSrc) {
      const shouldCheckImageAvailability =
        prevState.currentBackgroundImageSrc !== nextBackgroundImageSrc;
      if (
    ...
    if (prevState.size) {
      ...
      if (
        prevState.size.width === size.width &&
        prevState.size.height === size.height
      ) {
        return null;
      }
    }
  ...
  render() {
    return React.createElement(
      ...
      ({ size: newParentSize }) => {
        ...
        // Layout
        const invisibleItems = StackLayout.invisibleItemIndexes(
          children,
          placeholders
        );
        const padding = StackLayout.calcPaddingSize(props);
        const constraintValues = ConstraintValues.fromProperties(props);
        const minSize = ConstraintValues.toMinSize(
          constraintValues,
          parentSize
        );
        const minContentSize = Size.subtract(minSize, padding);
        const minChildrenSizes = StackLayout.calcMinChildSizes(
          children,
          minContentSize,
          placeholders
        );
        const autoSize = StackLayout.calcAutoSize(
          minChildrenSizes,
          props,
          invisibleItems
        );
        const size = ConstraintValues.toSize(
          constraintValues,
          parentSize,
          autoSize,
          null
        ...
        const freeSpaceForChildren = StackLayout.calcFreeSpace(
          size,
          autoSize,
          childFractions,
          props
        ...
        const childSizes = StackLayout.calcChildSizes(
          children,
          contentSize,
          freeSpaceForChildren,
          placeholders
        );
        const childRects = StackLayout.calcChildLayoutRects(
          childSizes,
          contentSize,
          props,
          invisibleItems
        ...
        return React.createElement(
          ...
          React.createElement(EmptyState, {
            ...
            insideUserCodeComponent: !props.__fromCodeComponentNode,
          })
        );
      }
    );
  ...
  layoutChildren(childRects, placeholders) {
    return React.Children.map(this.props.children, (child, index) => {
      ...
      const constraintProps = getMergedConstraintsProps(
        child.props,
        constraints
      );
      const props = Object.assign(Object.assign({}, constraintProps), {
        style: placeholders
          ? Object.assign(Object.assign({}, child.props.style), {
  ...
  layoutPlaceholders(childRects, placeholders) {
    ...
    return placeholders.sizes.map((size, index) => {
      ...
      return React.createElement(DeprecatedFrame, {
...
/** @internal */
DeprecatedStack.defaultProps = Object.assign(
  Object.assign(
    Object.assign(
      Object.assign({}, Layer.defaultProps),
  ...
  gap: {
    ...
    hidden: (props) => {
      return (
        ...
        ["space-between", "space-around", "space-evenly"].indexOf(
          props.distribution
        ) !== -1
      );
    },
