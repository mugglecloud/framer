...
export class DeprecatedStack extends Layer {
    ...
    render() {
        return (<ConsumeParentSize>
            ...
            return (<DeprecatedFrame {...frameProps}>
                            {this.layoutChildren(childRects, placeholders)}
                            {this.layoutPlaceholders(childRects, placeholders)}
                            <EmptyState title={"Drop items"} children={this.props.children} size={size} showArrow={false} hide={!!this.props.placeholders} insideUserCodeComponent={!props.__fromCodeComponentNode}/>
                        </DeprecatedFrame>);
        }}
            </ConsumeParentSize>);
    }
    ...
    layoutPlaceholders(childRects, placeholders) {
        ...
        return placeholders.sizes.map((size, index) => {
            ...
            return (<DeprecatedFrame key={`stack-placeholder-${index}`} top={rect.y} left={rect.x} width={rect.width} height={rect.height} style={{ WebkitTransition: "transform 0.2s ease-out" }}/>);
        });
    }
}
/** @internal */
