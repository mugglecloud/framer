...
export function EmptyState({ title = "Connect to content", children, size, hide, showArrow = true, insideUserCodeComponent = false, }) {
    ...
    if (insideUserCodeComponent && childCount === 0) {
        return <FrameWithMotion width={width} height={height} data-name="placeholder"/>;
    }
    ...
    return (<FrameWithMotion key={`empty-state`} className="framerInternalUI-canvasPlaceholder" top={0} left={0} width={width} height={height} style={{
        display: "flex",
        alignItems: "center",
        ...
        padding: "0 10px",
            ...
            {shouldShowTitle && <Title>{title}</Title>}
    ...
    return (<svg xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{
        ...
        marginTop: 1,
            ...
            <g transform="translate(0.5 0.5)">
                <path d="M 0 3 L 12 3" fill="transparent" stroke="currentColor" strokeLinecap="butt"/>
                <path d="M 9 0 L 12 3 L 9 6" fill="transparent" stroke="currentColor" strokeLinecap="butt"/>
            </g>
        </svg>);
};
