        ...
        return (React.createElement(FrameWithMotion, { top: 0, left: 0, width: "100%", height: "100%", position: "relative", style: Object.assign({ overflow: "hidden", background: "unset" }, this.props.style) },
            React.createElement(NavigationContext.Provider, { value: this },
                ...
                this.overlayStack.map((item, stackIndex) => {
                    ...
                    return (React.createElement(NavigationContainer, { key: item.key, position: item.transition.position, initialProps: initialPropsForContainer(stackIndex, overlayStackState), transitionProps: transitionPropsForContainer(stackIndex, overlayStackState), instant: isInstantContainerTransition(stackIndex, overlayStackState), animation: animationPropsForContainer(stackIndex, overlayStackState), visible: containerIsVisible(stackIndex, overlayStackState), hideAfterTransition: hideAfterTransition, backdropColor: backdropColorForTransition(item.transition), backfaceVisible: getBackfaceVisible(stackIndex, overlayStackState), onTapBackdrop: backdropTapAction(item.transition, hideAfterTransition, this.goBack) }, containerContent(item)));
                }))));
    }
}
