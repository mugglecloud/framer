...
export class NavigationContainer extends React.Component {
    ...
    render() {
        ...
        if (layout.top === undefined || layout.bottom === undefined)
            layout.height = "auto";
        return (<FrameWithMotion width={"100%"} height={"100%"} style={{
            position: "absolute",
            transformStyle: "flat",
            ...
            visibility: visible ? "visible" : "hidden",
                ...
                <FrameWithMotion width={"100%"} height={"100%"} transition={transition} initial={{ opacity: instant && visible ? 1 : 0 }} // To prevent flashing
         animate={{ opacity: hideAfterTransition ? 0 : 1 }} background={backdropColor ? backdropColor : "transparent"} onTap={onTapBackdrop}/>
                <FrameWithMotion {...layout} initial={{ ...allAnimatableProperties, ...origins, ...initialProps }} animate={{ ...allAnimatableProperties, ...origins, ...transitionProps }} transition={{
            default: transition,
            originX: { type: false },
            originY: { type: false },
            originZ: { type: false },
                    ...
                    {this.props.children}
                    {hideAfterTransition && (<FrameWithMotion // Block pointer events from starting a new transition
