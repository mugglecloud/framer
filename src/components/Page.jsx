...
export function Page(props) {
    ...
    const snapToPage = useSnapToPage(offsetForPage, scrollControls, currentContentPageRef, contentOffsetRef, isHorizontal);
    ...
    const childComponents = React.Children.map(children, (child, index) => {
        ...
        return (<PageContainer key={index} effect={effectDictionary} dragEnabled={dragEnabled} direction={direction} contentHeight={contentHeight} contentWidth={contentWidth} alignment={alignment} gap={gap} isLastPage={index === pageCount - 1} contentOffset={contentOffsetRef.current} scrollControls={scrollControls} maxScrollOffset={maxOffset} directionLock={directionLock} onDragStart={onDragStartHandler} onDrag={onDragHandler} onDragEnd={onDragEndHandler}>
    ...
    return (<FrameWithMotion preserve3d={false} perspective={hasEffect(props) ? 1200 : undefined} overflow={overflow} {...containerProps} ref={containerRef}>
            <FrameWithMotion data-framer-component-type="Page" ref={scrollableRef} background={null} x={contentOffsetRef.current.x} y={contentOffsetRef.current.y} animate={scrollControls} width={"100%"} height={"100%"} preserve3d={true} style={{
        padding: makePaddingString(paddingFromProps(props)),
        display: "flex",
        flexDirection: isHorizontal ? "row" : "column",
    }}>
                <EmptyState children={children} size={{
        width: "100%",
        height: "100%",
...
function getPageContentRects(containerRef, containerSize, direction, gap) {
    ...
    const contentWrappers = containerElement.querySelectorAll(`:scope > [data-framer-component-type="Page"] > [data-framer-component-type="PageContainer"] > [data-framer-component-type="PageContentWrapper"]`);
    const sizes = [];
...
function useSnapToPage(offsetForPage, scrollControls, currentContentPageRef, contentOffsetRef, isHorizontal) {
